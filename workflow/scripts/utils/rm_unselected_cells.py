...
df_config = pd.read_csv(
    "{data_location}/config/config_df_ashleys.tsv".format(
...
df_config.loc[
    df_config["Cell"].isin([e.split(".")[0] for e in cells_unselected]), "Selected"
] = False
df_config.to_csv(
    "{data_location}/config/config_df_ashleys.tsv".format(
        data_location=snakemake.config["data_location"]
...
with open(snakemake.output[0], "w") as out:
    out.write(
        "data_location processed: {data_location}\n".format(
            data_location=snakemake.params.path
        )
    )
    ...
    for cell in cells_unselected:
        ...
        subprocess.call(
            "rm {path}/{sample}/selected/{cell}".format(
                path=snakemake.params.path, sample=snakemake.wildcards.sample, cell=cell
            ),
            shell=True,
        )
        subprocess.call(
            "rm {path}/{sample}/selected/{cell}.bai".format(
                path=snakemake.params.path, sample=snakemake.wildcards.sample, cell=cell
            ),
            shell=True,
        )
