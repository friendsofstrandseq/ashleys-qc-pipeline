...
def get_ipysheet(cell_per_sample, sample, mosaic_info, ashleys_labels):
    ...
    final_df.loc[
        (final_df["prediction"] == 1) & (final_df["pass1"] == True), "final_pred"
    ] = 1
    ...
    # Cleaning
    ashleys_labels["prediction"] = ashleys_labels["prediction"].replace(
        {1: "✅", 0: "❌"}
    )
    ...
    column(
        2, mosaic_info_df["pass1"].replace({True: "✅", False: "❌"}).values.tolist()
    )
